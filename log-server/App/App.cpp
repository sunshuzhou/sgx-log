...
void ocall_listen_log_messages(){
  ...
  while(1) {
    count = count + 1;
    if (count == 10) {
      printf("Count has reached 10, exiting! Good bye!\n");
      break;
    }

    // Keep listening before count reaches 10 or client requests for connection
...
uint32_t ocall_write_sealed_data( uint8_t *buffer, uint32_t buflen, const char * filename) {
  FILE* pFile;
