    ...
    if (strstr((const char *) msg, (const char *) config[i].facility)
      ...
      if (config[i].log_buffer_length < LOG_BUFFER_MAX_SIZE) {
        for (int j = 0; j < log_length; j++) {
          //printf("\ncopying %d", j);
          config[i].log_buffer[j + config[i].log_buffer_length] = msg[j];
        }
      ...
      if (config[i].log_buffer_length < LOG_BUFFER_MAX_SIZE) {
        ...
        config[i].log_buffer_length += strlen(msg_mac);
        printf("\n Log buffer size:%d", config[i].log_buffer_length);
      }
      ...
      if (config[i].msg_count == MAX_BLOCK_SIZE && config[i].log_buffer_length < LOG_BUFFER_MAX_SIZE && seal == 1) {
        need_len = sgx_calc_sealed_data_size(0,
            config[i].log_buffer_length);
        uint8_t * sealed_data = (uint8_t*) seal_data(config[i].log_buffer, config[i].log_buffer_length);
        ...
        printf("\n Writing log buffer in file:%s", (char*) config[i].filename);

        if (ocall_write_sealed_data(&err, sealed_log_buf, need_len, (char*) config[i].filename)) {
      ...
      if ((config[i].msg_count == MAX_BLOCK_SIZE) && (config[i].log_buffer_length < LOG_BUFFER_MAX_SIZE) && seal == 0) {

        printf("\n Before log buffer size:%d", config[i].log_buffer_length);
        printf("\n Writing log buffer in file:%s", (char*) config[i].filename);

        config[i].log_buffer[config[i].log_buffer_length] = '\n';
        config[i].log_buffer_length++;

        if (ocall_write_sealed_data(&err, config[i].log_buffer, config[i].log_buffer_length, (char*) config[i].filename)) {
          printf("\nBuffer writing to file failed.\n");
          return;
        }
